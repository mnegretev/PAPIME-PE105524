...
\begin{frame}\frametitle{Tarea 08 - Cinemática Inversa}
  \begin{enumerate}
  \item En el archivo \texttt{catkin\_ws/src/students/scripts/assignment08.py}, en la función \texttt{forward\_kinematics}, implemente la el cálculo de la cinemática directa. Siga las instrucciones dadas en los comentarios.
  \item En la función, \texttt{jacobian}, calcule el jacobiano del manipulador para la configuración $q$ utilizando la aproximación por diferencias finitas. Siga las instrucciones de los comentarios del código.
  \item En la función \texttt{inverse\_kinematics\_xyzrpy}, implemente el cálculo de la cinemática inversa por el método de Newton-Raphson. Siga las instrucciones de los comentarios del código. 
  \item Ejecute la simulación con el comando \texttt{roslaunch surge\_et\_ambula justina\_gazebo.launch }
  \item Ejecute la tarea con el comando \texttt{rosrun students assingment08.py}
  \end{enumerate}
\end{frame}
...
\begin{frame}\frametitle{Tarea 08 - Cinemática Inversa}
  \begin{enumerate}
    \setcounter{enumi}{5}
  \item En las pestañas \textit{Left Arm} o \textit{Right Arm} de la GUI realice lo siguiente:
    \begin{enumerate}
    \item Seleccione la opción \textit{Send Single Pose}
    \item Escriba \textit{prepare} en el campo \textit{Goal} del grupo \textit{Arm Articular} y presione enter
    \item Presione el botón \textit{+X} y verifique que el brazo se mueva unos 10 cm hacia adelante
      \begin{figure}
        \centering
         \includegraphics[width=0.4\textwidth]{Figures/IK_Gui.png}
      \end{figure}
    \end{enumerate}
  \end{enumerate}
  \textbf{Entregables:}
  ...
  \end{itemize}
  \textbf{Deadline: } 2023-11-28 al inicio de la clase. 
\end{frame}
